...
export type IndexStatus = {
  version: 1; databasePath: string; latestRun: null | { completedAt: string | null; status: string };
  counts: { sources: number; threads: number; events: number; messages: number; diagnostics: number };
};

export function getIndexStatus(database: WorktrailDatabase, path: string): IndexStatus {
  const latestRun = database.raw.prepare(
  ...
  return {
    ...
    latestRun: latestRun ? { completedAt: latestRun.completed_at, status: latestRun.status } : null,
    counts: {
